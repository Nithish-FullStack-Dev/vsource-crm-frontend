...
export default function AllocatedLeadsPage() {
  ...
  const counselors = Array.from(
    new Set(seedLeads.map((item) => item.counselor)),
  );
  ...
  const filteredLeads = useMemo(() => {
    return allocatedLeads.filter((lead) => {
      const counselorMatch =
        counselorFilter === "all" ||
        !counselorFilter ||
        lead.counselor === counselorFilter;
      const branchMatch =
        branchFilter === "all" || !branchFilter || lead.branch === branchFilter;
      const statusMatch =
        statusFilter === "all" || !statusFilter || lead.status === statusFilter;
                  ...
                  >
                    ...
                    <td className="px-4 py-4 space-x-1 whitespace-nowrap">
                      ...
                      <Dialog
                        ...
                        onOpenChange={(open) =>
                          setReassignLead(open ? lead : null)
                        }
                      >
              ...
              <div className="space-y-4 px-4 py-3">
                ...
                <div className="rounded-2xl border border-border bg-background p-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    This lead is currently allocated to {selectedLead.counselor}{" "}
                    and is being handled in the {selectedLead.branch} branch.
                  </p>
                </div>
              </div>
            </>
