...
export default function TodayFollowupPage() {
  const [followups, setFollowups] = useState<Followup[]>(
    seedFollowups.filter(
      (item) => new Date(item.date) <= new Date(Date.now() + 86400000 * 1),
    ),
  );
  ...
  const addNote = () => {
    ...
    setFollowups((current) =>
      current.map((item) =>
        item.id === noteFollowup.id
          ? { ...item, notes: `${item.notes} ${noteText}` }
          : item,
      ),
    );
  ...
  return (
    <PageTransition>
        ...
        <Card>
          <CardContent className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Calls and messages waiting for today.
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Progress recorded by counselors.
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Urgent follow-ups that need catch-up.
            </p>
          </CardContent>
        </Card>
      </div>
                  ...
                  >
                    ...
                    <td className="px-4 py-4 space-x-1 whitespace-nowrap">
                      ...
                      <Dialog
                        ...
                        onOpenChange={(open) =>
                          setRescheduleFollowup(open ? item : null)
                        }
                      >
      ...
      <Sheet
        open={!!noteFollowup}
        onOpenChange={(open) => !open && setNoteFollowup(null)}
      >
        <SheetContent>
          {noteFollowup && (
            <>
